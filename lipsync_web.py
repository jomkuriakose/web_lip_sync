...
def change_video_audio(file_list, vtt_obj, audio_file, video_file, lang):
        ...
        try:
            ...
            for st, end, tmp_clip in audio_iterator:
                ...
                if num==1:
                    ...
                    main_audio_array=np.concatenate((main_audio_array,tmp_arry), axis=0)
            final_clip = ac.AudioArrayClip(main_audio_array, fps=fps)
            new_file_path = "./"+video_file.split("/")[-1].split(".")[0]
...
def tts_vtt(gender, lang, vtt_filepath, audio_filepath, video_filepath, transcription_file, tts_audio_folder):
    vtt_obj = webvtt.read(vtt_filepath)
    file_names = os.listdir(tts_audio_folder)
    file_list = prepend(file_names,tts_audio_folder+'/')
    # print(file_list)
    ...
    # change_video_audio(file_list, vtt_obj, audio_file, video_file, lang)
    # audio_file=audio_filepath
    # video_file=video_filepath

    result_video_path = change_video_audio(file_list, vtt_obj, audio_filepath, video_filepath, lang)
...
if __name__ == "__main__":
    ...
    vtt_filepath = "./W1L1/input.vtt"
    audio_filepath = "./W1L1/input.wav"
    video_filepath = "./W1L1/input.mp4"
    transcription_file = "./W1L1/transcription"
    tts_audio_folder = "./W1L1/audio_W1L1"

    # tts_vtt(gender, lang, vtt_filepath, audio_filepath, video_filepath, transcription_file, tts_audio_folder)
    # gender="male"
    # lang="Hindi"

    vid_path = tts_vtt("male", "Hindi", vtt_filepath, audio_filepath, video_filepath, transcription_file, tts_audio_folder)
    print(vid_path)

# ffmpeg -i input.mp4 -acodec pcm_s16le -ac 2 input.wav
# python srt_2_vtt.py input.srt
# python lipsync_web.py
